...
local function remember_world_map_layer(layer)
    ...
    for _, known in ipairs(known_layers) do
        if known == layer then
            return
        end
    end
...
local function get_map_data(map_id)
    ...
    for _, object in ipairs(objects) do
        ...
        local values_ok, data = pcall(function()
            if tonumber(info.MapID) ~= map_id then
                return nil
            end
            return {
                map_id = map_id,
...
function WorldMap.initialize(log, is_current_generation)
    ...
    if not notify_ok then
        log("World-map creation observer failed: "
...
function WorldMap.read_state()
    ...
    if not position_ok or not viewport_ok or not viewport_scale_ok
        or not values_ok or state == nil
        or state.left == nil or state.top == nil
        or state.zoom == nil or state.zoom <= 0.0
        or state.viewport_width == nil or state.viewport_width <= 0.0
        or state.viewport_height == nil or state.viewport_height <= 0.0
        or state.viewport_scale == nil or state.viewport_scale <= 0.0
        or state.player_map_x == nil or state.player_map_y == nil
    then
        return nil
    end
    return state
